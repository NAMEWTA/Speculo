...
- **workflows**：I-to-issues 工作流大幅强化切片计划纪律——引入六段高质量 plan 结构（当前现状 → 需读文件 → 需改文件 → 数据库表 → 实现要点 → 关键决策），新增铁律「没有精确到文件路径的改动清单，不算垂直切片」。
  - `I-to-issues.md`：新增铁律段、内置文档引用；质量准则扩展为文件级精准 + 数据库可追溯 + 实现要点前置 + 关键决策收口；独立进入流程委托给 `issues-slices.md` 的深度搜索协议。
  - `issues-slices.md`：§0 新增「当前现状」段；新增 §2.5 涉及的数据库表；§3 每切片新增需阅读/需修改文件表 + 实现要点；新增 §5.5 关键决策；完成准则同步扩展。
  - `issues-slices-template.md`：骨架同步新增所有新段 `[TODO:]`，头部 blockquote 标注六段纪律。
...
- **vendor**：新增 `officecli` 技能——用于创建、检查、校对和修改 Office 文档（.docx、.xlsx、.pptx）的 AI 友好型 CLI 工具。
