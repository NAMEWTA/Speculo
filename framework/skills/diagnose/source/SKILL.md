---
name: diagnose
description: 针对疑难 Bug 和性能回退的纪律化诊断循环。复现 → 最小化 → 假设 → 插桩 → 修复 → 回归测试。当用户说「diagnose this」/「debug this」、报告 Bug、说某处出错/抛异常/失败，或描述性能回退时使用。
---
...
10. **HITL bash 脚本。** 最后手段。如果必须由人类点击，用 `scripts/hitl-loop.template.sh` 来驱动他们，使循环仍然结构化。捕获的输出反馈给你。
