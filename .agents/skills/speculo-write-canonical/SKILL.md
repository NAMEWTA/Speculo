...
2. 审计目标能力目录：确认能力类型（skill/command/workflow），识别主入口文件（SKILL.md / WORKFLOW.md / command .md），列出所有应包含的源文件，标记应排除的运行时状态目录（`_state/`、`.speculo/`、`.gitkeep`）。完成标准：源文件清单完整且无遗漏，排除列表正确，每个文件都有明确包含/排除理由。
...
- **总是包含**：主入口 .md、`references/**/*.md`、`routes/**/*.md`、`atomic-skills/**/*.md`、`PERSISTENCE.md`、`assets/**/*.json`、`_templates/**/*.md`。
