...
- `id` — 能力标识符。skill/command 取 frontmatter `id`；workflow 取 WORKFLOW.md frontmatter `id`；vendor skill 取 `name`。
...
- `order` — 从 1 开始连续递增的整数。主入口文件（`SKILL.md` / `WORKFLOW.md` / 单文件 command `.md`）始终为 `1`。其余文件按 references → routes → atomic-skills → assets → scripts 分组后字母序排列。
...
|----------|------|
| `SKILL.md` | skill 主入口 |
| `WORKFLOW.md` | workflow 主入口 |
| `PERSISTENCE.md` | workflow 运行时契约 |
...
| vendor `SKILL.md` | 仅当 canonical 化整个 workflow 时，将引用的 vendor skill 作为附加 `<source-file>` 包含或记录 `vendor-mount` 说明 |
