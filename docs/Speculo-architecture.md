...
│   │   ├── .claude/
│   │   │   └── commands/                     # /speculo-dev /speculo-doc 等触发文件
│   │   └── README.md                         # 接入步骤
...
│   │   │   └── ...
│   │   ├── 01-prd/
│   │   │   ├── 01-prd.md                     # 工作流执行指引（带 frontmatter）
│   │   │   ├── prd-core.md                   # Step 1 规范
│   │   │   └── prd-review.md                 # Step 2 规范
│   │   ├── 02-design/
│   │   ├── 03-implement/
│   │   ├── 04-test/
│   │   └── 05-review/
...
│       └── 02-deploy/
│
├── skills/                                   # AgentSkills 规范的原子能力
│   └── <skill-name>/
│       ├── SKILL.md                          # 入口（强制命名 + 必填 frontmatter）
│       ├── references/                       # 渐进披露细节（按需加载）
│       ├── scripts/                          # 可选辅助脚本
│       └── examples/                         # 可选示例
│
└── .speculo/                                 # 项目持久化骨架
...
```markdown
<!-- workflows/dev/01-prd/01-prd.md 正文片段 -->

## 状态扩展字段
本工作流需在 `.status.json` 追加：
- `stakeholders` (array) — 干系人清单
