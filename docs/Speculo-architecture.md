...
| 当前内置 | status/archive/config-prune/生产力命令 | dev + doc + person 工作流 | command / workflow 可调用的能力 |
...
| 版本 | 日期 | 摘要 |
|------|------|------|
| v2.6 | 2026-07-03 | `dev/D-docs-sync` 升级为 tracked assets + archive 知识提取 + `.config` 生命周期审计；新增 `config-prune` dry-run 命令和 ADR supersession 元数据规则 |
