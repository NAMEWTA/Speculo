...
产物归档至：`speculo/.speculo/commands/<YYYY-MM-DD>-<command>-<topic>/`
...
如需复用 skill，在正文 `## 调用的 skills` 中用相对路径列出，并说明何时读 `SKILL.md`。skill 输出归档到本 command 的产物目录；skill 自身不写 `speculo/.speculo/`。
...
参考 `speculo/commands/archive.md` 和 `speculo/commands/config-prune.md`。
