...
| 命令 | 用途 |
|------|------|
| `speculo init [target]` | 安装到 `speculo/` 目录下；若 `speculo/` 不存在则全新安装全部资产（冲突时失败），若已存在则更新 `commands/`、`skills/`、`workflows/`，按模式处理 `vendor/`，并保留 `.speculo/` 状态 |
...
| ID | 入口 | 一句话用途 |
|----|------|----------|
| `dev/index` | `workflows/dev/AGENTS.md` | 智能导航 + 状态汇报 + 推荐下一步 |
...
| `dev/M-domain-modeling` | `workflows/dev/M-domain-modeling/M-domain-modeling.md` | `dev/M`：主动领域建模，维护 CONTEXT 通用语言与 ADR |
| `dev/A-improve-architecture` | `workflows/dev/A-improve-architecture/A-improve-architecture.md` | `dev/A`：架构深化机会扫描、HTML 审查报告与候选质询 |
| `dev/D-docs-sync` | `workflows/dev/D-docs-sync/D-docs-sync.md` | `dev/D`：基于 git diff、归档产物和 `.config` 生命周期同步文档/知识资产 |

### doc

| ID | 入口 | 一句话用途 |
|----|------|----------|
| `doc/index` | `workflows/doc/AGENTS.md` | 文档写作、塑形、编辑与素材管理导航 |
...
| ID | 入口 | 一句话用途 |
|----|------|----------|
| `person/index` | `workflows/person/AGENTS.md` | 人物方法论工作流导航 |
| `person/M-mao-zedong-cognitive-os` | `workflows/person/M-mao-zedong-cognitive-os/M-mao-zedong-cognitive-os.md` | `person/M`：以毛泽东方法论为底座的认知咨询 |

### state

| ID | 入口 | 一句话用途 |
|----|------|----------|
| `state/index` | `.speculo/AGENTS.md` | 状态骨架、项目知识和归档读取顺序 |
| `state/archive` | `.speculo/archive/AGENTS.md` | 已完成 change 的历史归档读取指引 |
...
| `config-prune` | `commands/config-prune.md` | dry-run 审计 `.config` 中可删除或合并的过期知识资产 |
