...
4. 报告 active 数量、各 change 的 `current_work`、去重后的 `works_run`、change 业务状态、最近更新时间、调查 claims，以及停滞 change（`.status.json` 超过 14 天未更新）。不解析或展示全局 `result`、`work_history` 或 `completed`。
