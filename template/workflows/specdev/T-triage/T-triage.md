...
workflow: specdev
name: 请求分诊
description: 需要冻结外部来源、审计摄入、对 completed change 回写来源 Issue，或把已完成 Ticket 投影为带分类标签的 GitHub Issue 时使用；已清晰的本地需求不必为了路由而经本入口。
...
Triage 是 SpecDev 唯一的远程摄入、关闭与发布投影边界。开发期间，`<Path>{roots.state}/specdev/changes/{change}/source.md</Path>`、`<Path>{roots.state}/specdev/changes/{change}/triage.md</Path>`、`<Path>{roots.state}/specdev/changes/{change}/publish.md</Path>`、Spec、Ticket、Map、Goal Plan、Evidence 和状态文件是唯一权威；远程系统只保存原始请求以及经确认后的完成通知与票级发布记录。
...
`external_action` 只描述源 Issue。`publish_action` 只描述票级发布投影。两者互不覆盖。未授权时远程写入为零。
...
验证通过后原子重读 source、triage、publish（若请求过）和 change 状态。Intake、reconcile 或 publish 成功时将 `specdev/triage` 去重加入 `works_run` 并清空 `current_work`；可恢复失败保留 `current_work` 和具体 blocker。
