...
运行 ID：`bde8f8f47a90`；开始：2026-09-14T08:44:02.758384+00:00；完成：2026-09-14T08:44:02.766677+00:00。

## 1. 结论与空间口径

系统：**Windows / SIMULATED_ARCHITECTURE**。本次发现 6 条待复核事项；可选隔离候选 3 项，自动永久删除动作 **0** 项。检测没有启动包管理器、连接网络、卸载软件或修改环境配置。
...
| F0003 | R2 | java 有多个 PATH 命中 | \["~/bin-primary/java.exe", "~/bin-old/java.exe"\] | 在可信终端核实实际版本和主用管理器；多个路径不等于冗余安装。 |
...
| --- | --- | --- | --- |
| java | ~/bin-primary/java.exe | \["~/bin-primary/java.exe", "~/bin-old/java.exe"\] | 未执行，未知 |
| javac | — | \[\] | 未执行，未知 |
| mvn | — | \[\] | 未执行，未知 |
| mvnw | — | \[\] | 未执行，未知 |
| gradle | — | \[\] | 未执行，未知 |
...
| uv-cache | ~/AppData/Local/uv/cache | absent | 未知 | R2 | known default hypothesis; not proof of active use — Use native uv cache commands after version verification; no automated moves. |
| pnpm-store | ~/AppData/Local/pnpm/store | absent | 未知 | R2 | known default hypothesis; not proof of active use — Version-dependent default hypothesis; use pnpm store path to establish actual location. |
| npm-home | ~/AppData/Local/npm-cache | measured | 52.00 KiB | R2 | known default hypothesis; not proof of active use — May contain logs and other state; only \_cacache can qualify for quarantine. |
...
| fnm | ~/AppData/Local/fnm | absent | 未知 | R2 | known default hypothesis; not proof of active use — Default hypothesis, verify with configuration. |
| mise | ~/AppData/Local/mise | absent | 未知 | R2 | known default hypothesis; not proof of active use — Support and path semantics must be checked against installed version. |
...
| npm-content-cache | ~/AppData/Local/npm-cache/\_cacache | measured | 52.00 KiB | R1 | conservative default cache location — Recreation may need network; metadata age does not prove disuse. |
| pip-cache | ~/AppData/Local/pip/Cache | measured | 52.00 KiB | R1 | conservative default cache location — Recreation may need network; metadata age does not prove disuse. |
| go-build-cache | ~/AppData/Local/go-build | measured | 52.00 KiB | R1 | conservative default cache location; process environment: GOCACHE — Recreation may need network; metadata age does not prove disuse. |
...
| --- | --- | --- | --- | --- |
| Example Editor | 1.0-fixture | HKCU/Uninstall/FIXTURE-EXAMPLE-ID | False | fixture candidate; not real software and never uninstalled |
...
| 文件 | 逻辑大小 | 修改时间 | 处理 |
| --- | --- | --- | --- |
| ~/Downloads/old-installer.zip | 17.00 B | 2026-09-14T08:44:02.758127+00:00 | manual ownership/backup/content review; not a deletion candidate |
...
| --- | --- | --- | --- | --- |
| Q-0dfe5f6cad | npm-content-cache | ~/AppData/Local/npm-cache/\_cacache | 52.00 KiB | 否 |
| Q-75b854ace3 | pip-cache | ~/AppData/Local/pip/Cache | 52.00 KiB | 否 |
| Q-f87c9bcd73 | go-build-cache | ~/AppData/Local/go-build | 52.00 KiB | 否 |
