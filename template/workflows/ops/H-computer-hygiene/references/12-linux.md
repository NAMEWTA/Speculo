...
建议按 XDG 的配置、数据、缓存和状态语义组织路径。统一 DevRoot 可设为 `~/.local/share/dev`，但这是工程选择，不应无条件替换发行版与系统服务规定的路径。系统 Python、`/usr`、`/lib`、`/etc`、包数据库、内核和 initramfs 默认保护。[XDG-1](00-sources.md#xdg-1)
...
WSL 报告只代表该发行版。宿主 Windows PATH 注入、Windows 可执行文件、`/mnt/c`、Linux 原生 SDK 和虚拟磁盘是不同层级；项目和缓存是否位于宿主挂载卷须记录。释放 Linux 文件与缩小宿主 vhdx 不是同一个动作，后者按 Microsoft 的当前指导另行确认。[WIN-6](00-sources.md#win-6)
