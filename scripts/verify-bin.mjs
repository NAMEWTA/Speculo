...
try {
  ...
  if (!/speculo \[init\] \[target\]/.test(out) || !/speculo version/.test(out) || !/speculo doctor/.test(out) || !/speculo recover/.test(out) || !/speculo resolve/.test(out)) {
...
console.log("verify-bin: OK - dist/src/cli.js exposes init, version, doctor, resolve, and explicit recover through --help.");
